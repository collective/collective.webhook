...
lint: ## Check and fix code base according to Plone standards
	...
	@$(RUFF) check --fix --config $(BACKEND_FOLDER)/pyproject.toml
	@uvx pyroma@latest -d .
...
format: ## Check and fix code base according to Plone standards
	...
	@$(RUFF) check --select I --fix --config $(BACKEND_FOLDER)/pyproject.toml
	@$(RUFF) format --config $(BACKEND_FOLDER)/pyproject.toml
...
i18n: $(VENV_FOLDER) ## Update locales
	...
	@uv run python -m collective.webhook.locales
