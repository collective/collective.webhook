...
class WebhookTests(unittest.TestCase):
    ...
    def setUp(self):
        tarball = create_tarball(os.path.join(os.path.dirname(__file__), "profile_01"))
    ...
    def test_rule_imported(self):
        ...
        self.assertEqual(rule.title, "HTTP GET")
    ...
    def test_execute_rules(self):
        ...
        self.assertEqual({"url": "http://nohost/plone/section"}, post[0][1]["json"])

        self.assertEqual("http://localhost:8080/", form[0][0][0])
        self.assertEqual({"url": "http://nohost/plone/section"}, form[0][1]["data"])
